...
class ResultadosEnquetesStruct extends FFFirebaseStruct {
  ResultadosEnquetesStruct({
    String? surveyId,
    String? pergunta,
  ...
  static ResultadosEnquetesStruct fromMap(Map<String, dynamic> data) =>
      ResultadosEnquetesStruct(
  ...
  static ResultadosEnquetesStruct? maybeFromMap(dynamic data) => data is Map
      ? ResultadosEnquetesStruct.fromMap(data.cast<String, dynamic>())
      : null;
  ...
  static ResultadosEnquetesStruct fromSerializableMap(
  ...
  @override
  String toString() => 'ResultadosEnquetesStruct(${toMap()})';

  @override
  bool operator ==(Object other) {
    const listEquality = ListEquality();
    return other is ResultadosEnquetesStruct &&
...
ResultadosEnquetesStruct createResultadosEnquetesStruct({
  String? surveyId,
  String? pergunta,
  int? totaisRespostas,
...
ResultadosEnquetesStruct? updateResultadosEnquetesStruct(
  ResultadosEnquetesStruct? resultadosEnquetes, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    resultadosEnquetes
...
void addResultadosEnquetesStructData(
  ...
  if (resultadosEnquetes == null) {
    return;
  }
  if (resultadosEnquetes.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && resultadosEnquetes.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final resultadosEnquetesData =
      getResultadosEnquetesFirestoreData(resultadosEnquetes, forFieldValue);
  final nestedData =
      resultadosEnquetesData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields =
      resultadosEnquetes.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getResultadosEnquetesFirestoreData(
  ResultadosEnquetesStruct? resultadosEnquetes, [
  bool forFieldValue = false,
]) {
  if (resultadosEnquetes == null) {
  ...
  final firestoreData = mapToFirestore(resultadosEnquetes.toMap());

  // Add any Firestore field values
  resultadosEnquetes.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getResultadosEnquetesListFirestoreData(
  List<ResultadosEnquetesStruct>? resultadosEnquetess,
) =>
    resultadosEnquetess
        ?.map((e) => getResultadosEnquetesFirestoreData(e, true))
        .toList() ??
    [];
